...
- Executes:
  ...
  ```
- Creates `physicians`, `cms_payments`, `disputes`, and `audit_tokens` tables.
- Converts `cms_payments` into a TimescaleDB hypertable partitioned on `payment_date`.
- Creates pgvector HNSW index on `cms_payments.product_embedding` and `physicians.specialty_embedding`.
...
- `pipeline_state` is persisted on `audit_tokens` in Tiger Data and queried live by the Campaign Console.
