...
type SortKey = 'name' | 'total_amount' | 'outlier_count';
...
export function DoctorTable({ physicians, selectedNpi, onSelect }: DoctorTableProps) {
  ...
  const sorted = useMemo(() => [...physicians].sort((a, b) => {
    ...
    return typeof left === 'string' && typeof right === 'string' ? (sortDir === 'asc' ? left.localeCompare(right) : right.localeCompare(left)) : (sortDir === 'asc' ? Number(left ?? 0) - Number(right ?? 0) : Number(right ?? 0) - Number(left ?? 0));
